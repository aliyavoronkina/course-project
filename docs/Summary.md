...
🔍 **Обнаружены критические дефекты в приложении:**
- **96% тестов не прошли** (24 из 25)
- **Категория:** Product defects (дефекты продукта)
- **Время выполнения:** 4 минуты 37 секунд
- **Основные проблемы:**
  - Данные не сохраняются в БД
  - Не работают уведомления
  - Неполная валидация полей
...
| Успешных | 1 (4%) |
| Неуспешных | 24 (96%) |
| Время выполнения | 4m 37s |
| Тестовых наборов | 3 |

**Тестовые наборы:**
- InvalidTest: 21 тест (валидация)
- DebitTest: 2 теста (дебетовая карта)
- CreditTest: 1 тест (кредит)
